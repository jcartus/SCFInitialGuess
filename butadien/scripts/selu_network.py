...
class AbsoluteIdempotencyPenalty(AbsoluteError):
    ...
    def function(self, network, y_placeholder):
        ...
        error = \
            super(AbsoluteIdempotencyPenalty, self).function(network, y_placeholder)
...
def main():   
    ...
    trainer = Trainer(
        SeluTrNNN(
            [dim**2, 700, 700, 700, 700, dim**2], 
            log_histograms=True
        ),
        error_function=AbsoluteError(),
        #cost_function=RegularizedMSE(alpha=1e-7),
        #cost_function=IdempotencyPenalty(dataset.inverse_input_transform, coupling=1e-5),
        cost_function=AbsoluteIdempotencyPenalty(dataset.inverse_input_transform, coupling=1e-3),
        optimizer=tf.train.AdamOptimizer(learning_rate=1e-3),
        gradient_clipping=[1e-5,1e-2]
    )
