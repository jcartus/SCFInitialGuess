...
def main(data_folder="butadien/data/", index_file=None):        
    ...
    msg.info("Index ...", 1)
    np.save(data_folder + "index400.npy", index)

    msg.info("S & P ...", 1)
    np.save(data_folder + "S400.npy", np.array(S).reshape(-1, dim**2))
    np.save(data_folder + "P400.npy", np.array(P).reshape(-1, dim**2))
    np.save(data_folder + "F400.npy", np.array(F).reshape(-1, dim**2))

    msg.info("Molecules ...", 1)
    np.save(data_folder + "molecules400.npy", molecules)
